...
def main():
    """主函数 - 直接分析PDF中的公司资质类要求"""
    # API密钥配置
    api_key = "sk-fe0485c281964259b404907d483d3777"
    
    # 检查PDF文件是否存在
    pdf_file = "/Users/leojiang/PycharmProjects/标书生成/03.招标文件.pdf"
    ...
    try:
        ...
        if choice == "1":
            ...
            try:
                ...
                print(f"将分析第 {start_page} 页到第 {end_page} 页...")
            except ValueError:
                print("页码输入错误，将分析全部页面")
        elif choice == "3":
            try:
                page_num = int(input("请输入要分析的页码: "))
                start_page = page_num
                end_page = page_num
                print(f"将分析第 {page_num} 页...")
            except ValueError:
                print("页码输入错误，将分析全部页面")
        else:
            print("选择无效，将分析全部页面")
    ...
    try:
        # 直接调用analyze_company_qualification_requirements作为主函数
        results = extractor.analyze_company_qualification_requirements(start_page=start_page, end_page=end_page)
        
        # 生成报告
        report = extractor.generate_qualification_report(start_page=start_page, end_page=end_page)
        print("\n" + report)
        ...
        if start_page and end_page:
            if start_page == end_page:
                page_suffix = f"_page_{start_page}"
            else:
                page_suffix = f"_pages_{start_page}-{end_page}"
        
        report_path = pdf_file.replace('.pdf', f'_qualification_requirements_report{page_suffix}.txt')
        try:
            with open(report_path, 'w', encoding='utf-8') as f:
                f.write(report)
            print(f"\n公司资质类要求报告已保存到: {report_path}")
        except Exception as e:
            print(f"保存报告失败: {e}")
